  ...
  stages {
    ...
    stage('Push to Nexus') {
      steps {
        script {
          def BACKEND_REPOS = [{{backendRepos}}]
          def projectRepo = "${REGISTRY_URL}/${SONAR_PROJECT_KEY}"

          docker.withRegistry("http://${REGISTRY_URL}", "NEXUS") {
            def frontendTag = "${projectRepo}:frontend-${env.BUILD_ID}"
            sh "docker build -t ${frontendTag} ./frontend"
            sh "docker push ${frontendTag}"

            BACKEND_REPOS.each { repo ->
              def repoName = repo.tokenize('/').last().replace('.git', '')
              def tag = "${projectRepo}:${repoName}-${env.BUILD_ID}"
              sh "docker build -t ${tag} ${repoName}"
              sh "docker push ${tag}"
            }
          }
        }
    ...
    stage('Creating VM Infrastructure') {
      steps {
        script {
          echo "Creating with terraform programmatically..."
        }
      }
    }

    stage('Configurating VM Infrastructure') {
      steps {
        script {
          echo "Configuring with ansible programmatically..."
        }
      }
    }
    ...
    stage('Deploy to VM') {
      steps {
        echo "🚀 Deploying '${SONAR_PROJECT_KEY}' to VM"
      }
    }
  }
